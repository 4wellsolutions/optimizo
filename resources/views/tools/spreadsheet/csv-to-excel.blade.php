    ...
    <div class="mb-16 px-4">
        <div class="bg-white rounded-3xl p-8 md:p-10 shadow-xl border border-gray-100 mb-16">
            <div class="text-center mb-8">
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{{ __tool('csv-to-excel', 'form.title') }}</h2>
                <p class="text-lg text-gray-600">{{ __tool('csv-to-excel', 'form.subtitle') }}</p>
            </div>

            <form id="converterForm" method="POST" enctype="multipart/form-data" class="space-y-8" onsubmit="return false;">
                @csrf

                <div class="border-4 border-dashed border-green-100 rounded-3xl p-10 text-center hover:border-green-300 hover:bg-green-50 transition-all cursor-pointer relative group bg-gray-50/50" id="dropzone">
                    <input type="file" name="file" id="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".csv">
                    <div class="space-y-6 pointer-events-none">
                        ...
                        <div>
                            <p class="text-xl font-bold text-gray-700">{{ __tool('csv-to-excel', 'form.upload_label') }}</p>
                            <p class="text-base text-gray-500 mt-2">{{ __tool('csv-to-excel', 'form.upload_help') }}</p>
                        </div>
                    </div>
                ...
                <div class="text-center">
                    <button type="button" id="convertBtn" class="w-full md:w-auto min-w-[300px] inline-flex items-center justify-center px-10 py-5 border border-transparent text-xl font-bold rounded-2xl shadow-xl text-white bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 focus:outline-none focus:ring-4 focus:ring-green-500/50 transform hover:-translate-y-1 transition-all">
                        {{ __tool('csv-to-excel', 'form.button') }}
        ...
        <div class="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-gray-100 mb-12">
            <article class="prose prose-lg md:prose-xl max-w-none text-gray-600">
                <div class="text-center max-w-3xl mx-auto mb-16">
                    <h2 class="text-3xl md:text-4xl font-black text-gray-900 mb-6">{{ __tool('csv-to-excel', 'content.hero_title') }}</h2>
                    <p class="text-xl leading-relaxed text-gray-600">{{ __tool('csv-to-excel', 'content.hero_description') }}</p>
                </div>
                ...
                <div class="grid md:grid-cols-3 gap-8 mb-16 not-prose">
                    <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 hover:shadow-lg transition-shadow">
                        ...
                        <h3 class="font-bold text-2xl text-gray-900 mb-3">{{ __tool('csv-to-excel', 'content.feature1_title') }}</h3>
                        <p class="text-base text-gray-600 leading-relaxed">{{ __tool('csv-to-excel', 'content.feature1_desc') }}</p>
                    </div>
                    <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 hover:shadow-lg transition-shadow">
                        ...
                        <h3 class="font-bold text-2xl text-gray-900 mb-3">{{ __tool('csv-to-excel', 'content.feature2_title') }}</h3>
                        <p class="text-base text-gray-600 leading-relaxed">{{ __tool('csv-to-excel', 'content.feature2_desc') }}</p>
                    </div>
                    <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 hover:shadow-lg transition-shadow">
                        ...
                        <h3 class="font-bold text-2xl text-gray-900 mb-3">{{ __tool('csv-to-excel', 'content.feature3_title') }}</h3>
                        <p class="text-base text-gray-600 leading-relaxed">{{ __tool('csv-to-excel', 'content.feature3_desc') }}</p>
                    </div>
                </div>

                <div class="border-t border-gray-100 pt-12">
                    <h3 class="text-3xl font-bold text-center text-gray-900 mb-10">{{ __tool('csv-to-excel', 'faq.title') }}</h3>
                    <div class="grid md:grid-cols-2 gap-8">
                        <div>
                            <h4 class="font-bold text-gray-900 text-lg mb-2">{{ __tool('csv-to-excel', 'faq.q1') }}</h4>
                            <p class="text-gray-600">{{ __tool('csv-to-excel', 'faq.a1') }}</p>
                        </div>
                        <div>
                            <h4 class="font-bold text-gray-900 text-lg mb-2">{{ __tool('csv-to-excel', 'faq.q2') }}</h4>
                            <p class="text-gray-600">{{ __tool('csv-to-excel', 'faq.a2') }}</p>
                        </div>
                    </div>
                </div>
            </article>
...
(function() {
    ...
    convertBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route("utility.csv-to-excel.convert") }}', {
                method: 'POST',
                body: formData,
