...
@section('title', __tool('csv-to-sql', 'meta.title'))
@section('meta_description', __tool('csv-to-sql', 'meta.description'))
        ...
        <div class="bg-white rounded-3xl p-8 md:p-10 shadow-xl border border-gray-100 mb-16">
            ...
            <form id="converterForm" method="POST" enctype="multipart/form-data" class="space-y-8" onsubmit="return false;">
                ...
                <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                    <label
                        class="block text-gray-700 font-bold mb-2">{{ __tool('csv-to-sql', 'form.table_name_label') }}</label>
                    <input type="text" name="table_name"
                        placeholder="{{ __tool('csv-to-sql', 'form.table_name_placeholder') }}"
            ...
            <article class="prose prose-lg md:prose-xl max-w-none text-gray-600">
                <div class="text-center max-w-3xl mx-auto mb-16">
                    <h2 class="text-3xl md:text-4xl font-black text-gray-900 mb-6">
                        {{ __tool('csv-to-sql', 'content.hero_title') }}</h2>
                    <p class="text-xl leading-relaxed text-gray-600">{{ __tool('csv-to-sql', 'content.hero_description') }}
                    </p>
                </div>
                ...
                <div class="grid md:grid-cols-3 gap-8 mb-16 not-prose">
                    <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 hover:shadow-lg transition-shadow">
                        ...
                        <h3 class="font-bold text-2xl text-gray-900 mb-3">
                            {{ __tool('csv-to-sql', 'content.feature1_title') }}</h3>
                        <p class="text-base text-gray-600 leading-relaxed">
                            {{ __tool('csv-to-sql', 'content.feature1_desc') }}</p>
                    </div>
                    <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 hover:shadow-lg transition-shadow">
                        ...
                        <h3 class="font-bold text-2xl text-gray-900 mb-3">
                            {{ __tool('csv-to-sql', 'content.feature2_title') }}</h3>
                        <p class="text-base text-gray-600 leading-relaxed">
                            {{ __tool('csv-to-sql', 'content.feature2_desc') }}</p>
                    </div>
                    <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 hover:shadow-lg transition-shadow">
                        ...
                        <h3 class="font-bold text-2xl text-gray-900 mb-3">
                            {{ __tool('csv-to-sql', 'content.feature3_title') }}</h3>
                        <p class="text-base text-gray-600 leading-relaxed">
                            {{ __tool('csv-to-sql', 'content.feature3_desc') }}</p>
                    </div>
                </div>

                <div class="border-t border-gray-100 pt-12">
                    <h3 class="text-3xl font-bold text-center text-gray-900 mb-10">{{ __tool('csv-to-sql', 'faq.title') }}
                    </h3>
    ...
    <script>
        (function () {
            ...
            convertBtn.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch('{{ route("spreadsheet.csv-to-sql.convert") }}', {
                        method: 'POST',
                        body: formData,
