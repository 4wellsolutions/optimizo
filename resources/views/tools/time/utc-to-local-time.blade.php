...
@section('content')
    ...
    <div class="max-w-4xl mx-auto mb-16 px-4">
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
            <div class="bg-gray-50 px-8 py-6 border-b border-gray-100">
                <h3 class="text-xl font-bold text-gray-800">{{ __tool('utc-to-local-time', 'form.result_title') }}</h3>
                <p class="text-sm text-gray-500">{{ __tool('utc-to-local-time', 'form.help_text') }}</p>
            </div>
            
            <div class="p-8">
                <div class="mb-8">
                    <label class="block text-sm font-bold text-gray-700 mb-2">{{ __tool('utc-to-local-time', 'form.label') }}</label>
                    ...
                    <p class="text-xs text-gray-400 mt-2 ml-1">{{ __tool('utc-to-local-time', 'form.example') }}</p>
                </div>
                ...
                <div class="flex justify-end">
                    <button onclick="convertUtcToLocal()" class="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg transition-all hover:-translate-y-0.5">
                        {{ __tool('utc-to-local-time', 'form.button') }}
                    </button>
                </div>

                {{-- Result --}}
                <div id="resultBox" class="hidden mt-8 p-8 bg-indigo-50 rounded-2xl border border-indigo-100 animate-fade-in-up text-center">
                    <div class="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-2">{{ __tool('utc-to-local-time', 'form.result_title') }}</div>
    ...
    <article class="max-w-4xl mx-auto prose prose-lg prose-indigo px-4 mb-20">
        <div class="bg-gradient-to-br from-white to-gray-50 rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100">
            <h2 class="text-3xl font-extrabold text-gray-900 mb-6 font-display">{{ __tool('utc-to-local-time', 'content.hero_title') }}</h2>
            <p class="lead text-gray-600">{{ __tool('utc-to-local-time', 'content.hero_desc') }}</p>
            ...
            <h3 class="flex items-center gap-3 text-2xl font-bold text-gray-900 mt-12 mb-6">
                ...
                {{ __tool('utc-to-local-time', 'content.why_title') }}
            </h3>
            ...
            <div class="space-y-6 not-prose mb-12">
                <div class="flex gap-4 items-start">
                    ...
                    <div>
                        <h4 class="text-lg font-bold text-gray-900">{{ __tool('utc-to-local-time', 'content.consistency_title') }}</h4>
                        <p class="text-gray-600">{{ __tool('utc-to-local-time', 'content.consistency_desc') }}</p>
                    </div>
                </div>
                <div class="flex gap-4 items-start">
                     <div class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold flex-shrink-0">2</div>
                    <div>
                        <h4 class="text-lg font-bold text-gray-900">{{ __tool('utc-to-local-time', 'content.universality_title') }}</h4>
                        <p class="text-gray-600">{{ __tool('utc-to-local-time', 'content.universality_desc') }}</p>
                    </div>
                </div>
                 <div class="flex gap-4 items-start">
                    ...
                    <div>
                        <h4 class="text-lg font-bold text-gray-900">{{ __tool('utc-to-local-time', 'content.simplicity_title') }}</h4>
                        <p class="text-gray-600">{{ __tool('utc-to-local-time', 'content.simplicity_desc') }}</p>
                    </div>
                </div>
            </div>

            <h3 class="font-bold text-gray-900">{{ __tool('utc-to-local-time', 'content.offsets_title') }}</h3>
            <p>{{ __tool('utc-to-local-time', 'content.offsets_intro') }}</p>
            <ul>
                <li>{{ __tool('utc-to-local-time', 'content.offset_minus') }}</li>
                <li>{{ __tool('utc-to-local-time', 'content.offset_plus') }}</li>
                <li>{{ __tool('utc-to-local-time', 'content.offset_half') }}</li>
            </ul>

            <div class="mt-8 p-6 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-xl">
                 <h4 class="text-yellow-800 font-bold m-0 mb-2">{{ __tool('utc-to-local-time', 'content.golden_rule_title') }}</h4>
                 <p class="text-yellow-700 m-0 text-sm">{{ __tool('utc-to-local-time', 'content.golden_rule_desc') }}</p>
            </div>
        </div>
    </article>
