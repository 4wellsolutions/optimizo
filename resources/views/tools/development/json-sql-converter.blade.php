...
@section('title', __tool('json-sql-converter', 'meta.title'))
@section('meta_description', __tool('json-sql-converter', 'meta.description'))
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-2xl p-6 md:p-8 shadow-2xl border-2 border-blue-200 mb-8">
            ...
            <div class="flex gap-3 mb-6">
                <button onclick="setMode('encode')" id="encodeBtn"
                    ...
                    {{ __tool('json-sql-converter', 'editor.btn_encode') }}
                </button>
                <button onclick="setMode('decode')" id="decodeBtn"
                    ...
                    {{ __tool('json-sql-converter', 'editor.btn_decode') }}
                </button>
            </div>
            ...
            <div class="mb-6" id="tableNameDiv">
                <label for="tableName"
                    class="form-label text-base">{{ __tool('json-sql-converter', 'editor.label_table') }}</label>
                <input type="text" id="tableName" class="form-input"
                    placeholder="{{ __tool('json-sql-converter', 'editor.ph_table') }}" value="users">
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    <label for="numberInput" class="form-label text-base"
                        id="inputLabel">{{ __tool('json-sql-converter', 'editor.label_input_json') }}</label>
                    <textarea id="numberInput" class="form-input font-mono text-sm min-h-[300px]"
                        placeholder='{{ __tool('json-sql-converter', 'editor.ph_input_json') }}'></textarea>
                </div>

                <!-- Output Column -->
                <div>
                    <label for="numberOutput" class="form-label text-base"
                        id="outputLabel">{{ __tool('json-sql-converter', 'editor.label_output_sql') }}</label>
                    <textarea id="numberOutput" class="form-input font-mono text-sm min-h-[300px]" readonly
                        placeholder="{{ __tool('json-sql-converter', 'editor.ph_output_sql') }}"></textarea>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-3 mb-6">
                <button onclick="convertNumber()" class="btn-primary">
                    ...
                    <span id="processBtn">{{ __tool('json-sql-converter', 'editor.btn_convert_sql') }}</span>
                </button>
                <button onclick="clearAll()"
                    ...
                    <span>{{ __tool('json-sql-converter', 'editor.btn_clear') }}</span>
                </button>
                <button onclick="copyOutput()"
                    ...
                    <span>{{ __tool('json-sql-converter', 'editor.btn_copy') }}</span>
                </button>
            </div>
        ...
        <div
            ...
            <div class="text-center mb-8">
                ...
                <h2 class="text-4xl font-black text-gray-900 mb-3">{{ __tool('json-sql-converter', 'content.hero_title') }}
                </h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    {{ __tool('json-sql-converter', 'content.hero_subtitle') }}
                </p>
            </div>

            <p class="text-gray-700 leading-relaxed text-lg mb-8">
                {{ __tool('json-sql-converter', 'content.p1') }}
            </p>

            <h3 class="text-3xl font-bold text-gray-900 mb-6">
                {{ __tool('json-sql-converter', 'content.integration_title') }}</h3>
            <p class="text-gray-700 leading-relaxed mb-6">
                {{ __tool('json-sql-converter', 'content.integration_desc') }}
            </p>
            ...
            <h3 class="text-3xl font-bold text-gray-900 mb-6 font-primary">{{ __tool('json-sql-converter', 'content.samples_title') }}</h3>
            <div class="bg-white rounded-xl p-6 border-2 border-gray-200 mb-8">
                <div class="space-y-4">
                    @foreach(__tool('json-sql-converter', 'content.samples') as $key => $sample)
                        <div>
        ...
        <script>
            ...
            function setMode(mode) {
                ...
                if (mode === 'encode') {
                    ...
                    inputLabel.textContent = "{{ __tool('json-sql-converter', 'editor.label_input_json') }}";
                    outputLabel.textContent = "{{ __tool('json-sql-converter', 'editor.label_output_sql') }}";
                    processBtn.textContent = "{{ __tool('json-sql-converter', 'editor.btn_convert_sql') }}";
                    ...
                    inputLabel.textContent = "{{ __tool('json-sql-converter', 'editor.label_input_sql') }}";
                    outputLabel.textContent = "{{ __tool('json-sql-converter', 'editor.label_output_json') }}";
                    processBtn.textContent = "{{ __tool('json-sql-converter', 'editor.btn_convert_json') }}";
            ...
            function convertNumber() {
                ...
                if (!input) {
                    showStatus("{{ __tool('json-sql-converter', 'js.error_empty') }}", 'error');
                    return;
                }
                ...
                try {
                    if (currentMode === 'encode') {
                        // JSON to SQL
                        const tableName = document.getElementById('tableName').value.trim() || "{{ __tool('json-sql-converter', 'js.placeholder_table_name') }}";
                        let jsonData;
                        try {
                            jsonData = JSON.parse(input);
                        } catch (e) {
                            showStatus("{{ __tool('json-sql-converter', 'js.error_general') }}" + e.message, 'error');
                            return;
                        }

                        if (!Array.isArray(jsonData)) {
                            showStatus("{{ __tool('json-sql-converter', 'js.error_invalid_array') }}", 'error');
                            return;
                        }

                        if (jsonData.length === 0) {
                            showStatus("{{ __tool('json-sql-converter', 'js.error_empty_array') }}", 'error');
                            return;
                        }
                        ...
                        output.value = sql;
                        showStatus("{{ __tool('json-sql-converter', 'js.success_sql') }}", 'success');
                    } else {
                        ...
                        if (data.length === 0) {
                            showStatus("{{ __tool('json-sql-converter', 'js.error_no_statements') }}", 'error');
                            return;
                        }

                        output.value = JSON.stringify(data, null, 2);
                        showStatus("{{ __tool('json-sql-converter', 'js.success_json') }}", 'success');
                    }
                } catch (error) {
                    showStatus("{{ __tool('json-sql-converter', 'js.error_general') }}" + error.message, 'error');
                }
            }
            ...
            function copyOutput() {
                const output = document.getElementById('numberOutput');
                if (!output.value) {
                    showStatus("{{ __tool('json-sql-converter', 'js.error_no_copy') }}", 'error');
                    return;
                }
                output.select();
                document.execCommand('copy');
                showStatus("{{ __tool('json-sql-converter', 'js.success_copy') }}", 'success');
            }
