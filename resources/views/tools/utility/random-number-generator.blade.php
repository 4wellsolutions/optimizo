    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-2xl p-6 md:p-8 shadow-2xl border-2 border-purple-200 mb-8">
            <div class="grid md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label for="min" class="block text-sm font-semibold text-gray-700 mb-2">{{ __tool('random-number-generator', 'editor.label_min') }}</label>
                ...
                <div>
                    <label for="max" class="block text-sm font-semibold text-gray-700 mb-2">{{ __tool('random-number-generator', 'editor.label_max') }}</label>
            ...
            <div class="mb-6">
                <label for="count" class="block text-sm font-semibold text-gray-700 mb-2">{{ __tool('random-number-generator', 'editor.label_count') }}</label>
            ...
            <div class="flex flex-wrap gap-3 mb-6">
                <button onclick="generateRandom()"
                    ...
                    <span>{{ __tool('random-number-generator', 'editor.btn_generate') }}</span>
                </button>
            </div>

            <div class="mb-4">
                <label for="outputResult" class="block text-sm font-semibold text-gray-700 mb-2">{{ __tool('random-number-generator', 'editor.label_result') }}</label>
                <div id="outputResult"
        ...
        <div
            ...
            <h2 class="text-3xl font-black text-gray-900 mb-3 text-center">{{ __tool('random-number-generator', 'meta.h1') }}</h2>
            <p class="text-gray-600 text-center mx-auto max-w-xl">{{ __tool('random-number-generator', 'meta.subtitle') }}</p>
        </div>
    </div>
    ...
    <script>
        function generateRandom() {
            ...
            if (isNaN(min) || isNaN(max)) {
                showError("{{ __tool('random-number-generator', 'js.error_invalid') }}");
                return;
            }
            if (min > max) {
                showError("{{ __tool('random-number-generator', 'js.error_min_max') }}");
                return;
            }
