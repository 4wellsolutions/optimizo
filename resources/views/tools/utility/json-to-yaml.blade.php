...
@section('title', __tool('json-to-yaml', 'meta.h1'))
@section('meta_description', __tool('json-to-yaml', 'meta.subtitle'))
    ...
    <div class="max-w-6xl mx-auto">
        <div
            ...
            <div class="relative z-10 text-center">
                ...
                <h1 class="text-2xl md:text-3xl lg:text-4xl font-black text-white mb-2 leading-tight">
                    {{ __tool('json-to-yaml', 'content.hero_title') }}
                </h1>
                <p class="text-base md:text-lg text-white/90 font-medium max-w-3xl mx-auto">
                    {{ __tool('json-to-yaml', 'content.hero_subtitle') }}</p>
        ...
        <div class="bg-white rounded-2xl p-6 md:p-8 shadow-2xl border-2 border-cyan-200 mb-8">
            <div class="flex flex-wrap gap-3 mb-6">
                <button onclick="convert()" class="btn-primary"><svg class="w-5 h-5" fill="none" stroke="currentColor"
                    ...
                    </svg><span>{{ __tool('json-to-yaml', 'editor.btn_convert') }}</span></button>
                <button onclick="clearAll()"
                    ...
                    </svg><span>{{ __tool('json-to-yaml', 'editor.btn_clear') }}</span></button>
                <button onclick="copyOutput()"
                    ...
                    </svg><span>{{ __tool('json-to-yaml', 'editor.btn_copy') }}</span></button>
                <button onclick="loadExample()"
                    class="px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all font-semibold shadow-lg flex items-center gap-2"><svg
                        ...
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg><span>{{ __tool('json-to-yaml', 'editor.btn_example') }}</span></button>
            </div>
            <div id="statusMessage" class="hidden mb-6 p-4 rounded-xl font-semibold"></div>
            <div class="grid md:grid-cols-2 gap-6">
                <div>
                    <label for="jsonInput"
                        class="form-label text-base">{{ __tool('json-to-yaml', 'editor.label_input') }}</label>
                    <textarea id="jsonInput" class="form-input font-mono text-sm min-h-[400px]"
                        placeholder='{{ __tool('json-to-yaml', 'editor.ph_input') }}'></textarea>
                </div>
                <div>
                    <label for="yamlOutput"
                        class="form-label text-base">{{ __tool('json-to-yaml', 'editor.label_output') }}</label>
                    <textarea id="yamlOutput" class="form-input font-mono text-sm min-h-[400px]" readonly
                        placeholder="{{ __tool('json-to-yaml', 'editor.ph_output') }}"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-3xl p-8 md:p-12 border-2 border-cyan-100 shadow-2xl">
            <div class="text-center mb-8">
                ...
                <h2 class="text-4xl font-black text-gray-900 mb-3">{{ __tool('json-to-yaml', 'content.hero_title') }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">{{ __tool('json-to-yaml', 'content.hero_subtitle') }}
                </p>
            </div>

            <p class="text-gray-700 leading-relaxed text-lg mb-8">
                {{ __tool('json-to-yaml', 'content.p1') }}
            </p>

            <h3 class="text-3xl font-bold text-gray-900 mb-6">{{ __tool('json-to-yaml', 'content.what_title') }}</h3>
            <p class="text-gray-700 leading-relaxed mb-8">
                {{ __tool('json-to-yaml', 'content.what_desc') }}
            </p>

            <h3 class="text-3xl font-bold text-gray-900 mb-6">{{ __tool('json-to-yaml', 'content.features_title') }}</h3>
            ...
            <h3 class="text-3xl font-bold text-gray-900 mb-6">{{ __tool('json-to-yaml', 'content.uses_title') }}</h3>
            ...
            <h3 class="text-3xl font-bold text-gray-900 mb-6">{{ __tool('json-to-yaml', 'content.how_title') }}</h3>
            <div class="bg-white rounded-xl p-6 border-2 border-gray-200 mb-8 shadow-lg">
                <ol class="space-y-4">
                    @foreach($tool->content['how_steps'] as $step)
                        <li class="flex items-start">
                            <span
                                class="flex-shrink-0 w-8 h-8 bg-cyan-600 text-white rounded-full flex items-center justify-center font-bold mr-3">{{ $loop->iteration }}</span>
                            <div>
            ...
            <h3 class="text-3xl font-bold text-gray-900 mb-6 font-primary">{{ __tool('json-to-yaml', 'content.samples_title') }}</h3>
            <div class="bg-white rounded-xl p-6 border-2 border-gray-200 mb-8 shadow-lg">
                <div class="space-y-6">
                    @foreach(__tool('json-to-yaml', 'content.samples') as $key => $sample)
                        <div>
            ...
            <h3 class="text-3xl font-bold text-gray-900 mb-6">{{ __tool('json-to-yaml', 'content.faq_title') }}</h3>
            ...
            <h3 class="text-3xl font-bold text-gray-900 mb-6">{{ __tool('json-to-yaml', 'content.tips_title') }}</h3>
            <div class="bg-white rounded-xl p-6 border-2 border-gray-200 shadow-lg">
                <ul class="space-y-3">
                    @foreach($tool->content['tips_list'] as $tip)
                        <li class="flex items-start">
    ...
    @push('scripts')
        <script>
            function convert() {
                ...
                if (!input) { showStatus("{{ __tool('json-to-yaml', 'js.error_empty') }}", 'error'); return; }
                try {
                    const json = JSON.parse(input);
                    const yaml = jsonToYaml(json);
                    document.getElementById('yamlOutput').value = yaml;
                    showStatus("{{ __tool('json-to-yaml', 'js.success_convert') }}", 'success');
                } catch (error) { showStatus("{{ __tool('json-to-yaml', 'js.error_convert') }}" + error.message, 'error'); }
            }
            ...
            function copyOutput() { const output = document.getElementById('yamlOutput'); if (!output.value) { showStatus("{{ __tool('json-to-yaml', 'js.error_no_copy') }}", 'error'); return; } output.select(); document.execCommand('copy'); showStatus("{{ __tool('json-to-yaml', 'js.success_copy') }}", 'success'); }
