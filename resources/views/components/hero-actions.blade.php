...
<div
    class="flex flex-wrap items-center justify-between mt-2 pt-0 border-0 md:mt-6 md:pt-6 md:border-t md:border-white/20 gap-2 md:gap-4">
    <div class="flex items-center gap-2">
        <span class="text-xs md:text-sm font-semibold text-white/90">{{ __('common.share') }}:</span>
        <div class="flex gap-1 md:gap-2">
            ...
            <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($tool->meta_title ?? $tool->name ?? __('common.check_out_tool')) }}"
                target="_blank"
                class="inline-flex items-center justify-center w-8 h-8 md:w-10 md:h-10 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-lg transition-all duration-200"
            ...
            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" target="_blank"
                class="inline-flex items-center justify-center w-8 h-8 md:w-10 md:h-10 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-lg transition-all duration-200"
                title="{{ __('common.share_facebook') }}">
                <svg class="w-4 h-4 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(url()->current()) }}"
                target="_blank"
                class="inline-flex items-center justify-center w-8 h-8 md:w-10 md:h-10 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-lg transition-all duration-200"
                title="{{ __('common.share_linkedin') }}">
                <svg class="w-4 h-4 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    <button onclick="openReportModal()"
        class="inline-flex items-center justify-center w-8 h-8 md:w-auto md:h-10 md:px-6 gap-0 md:gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white rounded-lg transition-all duration-200 border border-white/30"
        title="{{ __('common.report_issue') }}">
        <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z">
            </path>
        </svg>
        <span class="hidden md:block">{{ __('common.report') }}</span>
    </button>
</div>
...
<div id="reportModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative">
        ...
        <h3 class="text-2xl font-bold text-gray-900 mb-4">{{ __('common.report_modal_title') }}</h3>
        ...
        <form id="reportForm" onsubmit="submitReport(event)">
            <div id="issueTypeSection" class="space-y-3 mb-6">
                <label class="block">
                    ...
                    <span class="text-gray-700 font-medium">{{ __('common.issue_not_working') }}</span>
                    <p class="text-sm text-gray-500 ml-6">{{ __('common.issue_not_working_desc') }}</p>
                </label>

                <label class="block">
                    <input type="radio" name="issueType" value="malfunctioned" class="mr-2" required>
                    <span class="text-gray-700 font-medium">{{ __('common.issue_malfunctioned') }}</span>
                    <p class="text-sm text-gray-500 ml-6">{{ __('common.issue_malfunctioned_desc') }}</p>
                </label>

                <label class="block">
                    <input type="radio" name="issueType" value="suggestion" class="mr-2" required>
                    <span class="text-gray-700 font-medium">{{ __('common.issue_suggestion') }}</span>
                    <p class="text-sm text-gray-500 ml-6">{{ __('common.issue_suggestion_desc') }}</p>
                </label>
            </div>

            <div id="suggestionSection" class="hidden mb-6">
                <label for="suggestionText" class="block text-sm font-semibold text-gray-700 mb-2">
                    {{ __('common.your_suggestion') }}
                </label>
                <textarea id="suggestionText" name="suggestion" rows="4"
                    ...
                    placeholder="{{ __('common.suggestion_placeholder') }}"></textarea>
            </div>

            <div class="mb-6">
                <label for="reportEmail" class="block text-sm font-semibold text-gray-700 mb-2">
                    {{ __('common.your_email') }} <span class="text-red-500">*</span>
                </label>
        ...
        <div id="reportSuccess" class="hidden text-center py-8">
            ...
            <h4 class="text-xl font-bold text-gray-900 mb-2">{{ __('common.thank_you') }}</h4>
            <p class="text-gray-600">{{ __('common.report_success') }}</p>
