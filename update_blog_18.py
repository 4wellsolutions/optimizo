...
images = {
    'featured': r'C:\Users\Ahmad\.gemini\antigravity\brain\9349fe3b-e0ab-45f1-abd3-2d47b93d32b8\blog_featured_image_allintitle_1769532193453.png',
...
BASE_URL = 'http://localhost:8000' # Change to local for development
...
for key, path in images.items():
    ...
    with open(path, 'rb') as f:
        ...
        if r.status_code == 201:
            ...
            print(f'Failed to upload {key}: {r.text}')
...
<img src="{urls['growth']}" alt="Success Chart" style="width:100%; border-radius: 10px; margin: 20px 0;">'''
    ...
    r = requests.put('https://optimizo.io/api/blogs/18', json=data)
    if r.status_code == 200:
        print('Blog post updated successfully!')
    else:
        print(f'Failed to update blog post: {r.text}')
else:
    print(f"Only {len(urls)}/4 images uploaded. Aborting update.")
